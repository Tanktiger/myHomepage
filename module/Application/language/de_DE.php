<?php
return array(
    '[NAVIGATION_HOME]' => 'Start',
    '[NAVIGATION_PROFILE]' => 'Profil',
    '[NAVIGATION_PROJECTS]' => 'Projekte',
    '[NAVIGATION_CONTACT]' => 'Kontakt',

    '[PERSON_HEADLINE]' => 'Zur Person',
    '[PERSON_HEADLINE_SMALL]' => 'Das bin ich',
    '[PERSON_TEXT_FIRST]' => 'Ich heiße Tom Scheduikat und bin',
    '[PERSON_TEXT_2]' => 'Jahre alt',
    '[PERSON_TEXT_3]' => 'Ich lebe zurzeit in Stralsund, einer schönen Stadt im Norden Deutschlands.</p>
                    <p class="lead">Zurzeit arbeite ich Hauptberuflich als Softwareentwickler für einen großen deutschen Internetkonzern.
                    Hauptsächlich verbringe ich meine Freizeit mit der Familie und kümmere mich um die anderen Sachen
                    wenn der Rest schläft. Dann programmiere ich an ein paar von
                    meinen eigenen <a href="#projects">Projekten</a> und wenn ich darauf mal keine Lust habe
                    dann spiele ich auch ein paar Spiele :D</p>',

    '[HELLO]' => 'Hallo',

    '[FACTS_HEADLINE]' => 'Ein Paar Fakten über mich',
    '[FACTS_PANEL_HEAD_1]' => 'Allgemein',
    '[FACTS_PANEL_HEAD_2]' => 'Qualifikationen',
    '[FACTS_PANEL_HEAD_3]' => 'Menschliches',
    'FACTS_PANEL_HUMAN_ATTR_1' => 'Teamfähig',
    'FACTS_PANEL_HUMAN_ATTR_2' => 'Kreativ',
    'FACTS_PANEL_HUMAN_ATTR_3' => 'Zielorientiert',
    'FACTS_PANEL_HUMAN_ATTR_4' => 'Organisator',

    '[PROJECTS_HEADLINE]' => 'Meine Projekte',
    '[PROJECTS_1_HEAD]' => 'Diese Homepage',
    '[PROJECTS_1_TEXT]' => 'Diese Homepage soll Infos zu meiner Person geben und für eventuell anstehende Bewerbungsgespräche dienen.',
    '[PROJECTS_2_HEAD]' => 'YuGiOh mDB',
    '[PROJECTS_2_TEXT]' => 'Dies ist meine erste Web App für das Smartphone. Mit jQuery Mobile und PhoneGap realisiert.<br>
                            Diese App sucht anhand von Namen oder Kartennummer, YuGiOh Karten aus einer DB und zeigt diese an.',
    '[PROJECTS_3_HEAD]' => 'Cardgame',
    '[PROJECTS_3_TEXT]' => ' Dies ist noch eine Geheime Idee von mir bezüglich eines Kartenspiels für Desktop und Mobile Bereich.',
    '[PROJECTS_4_HEAD]' => 'Ideasite',
    '[PROJECTS_4_TEXT]' => 'Eine Seite zum Austausch von Ideen wie Kickstartet.',

    '[CONTACT_HEADLINE]' => 'Kontakt',
    '[CONTACT_TEXT_1]' => 'Bitte kontaktiert mich wenn ihr Fragen habt.',
    '[CONTACT_TEXT_2]' => 'Oder schreibt mir einfach bei Xing.',

    '[APPLICATION_APPLICATIONTEXT]' =>
           '<p>Sehr geehrte Damen und Herren,</p><br>
            <p>wenn Sie einen Mitarbeiter suchen, der sich schnell in neue Herausforderungen einarbeiten kann, der für
                Probleme kreative sowie fachlich hervorragende Lösungen findet und jemanden, der zielstrebig der
                Firmenphilosophie nacheifert, dann sind sie jetzt fündig geworden!</p>
            <p>Zum gegenwärtigen Zeitpunkt arbeite ich als Junior Softwareentwickler bei der Unister GmbH in Stralsund.
                Gerne möchte ich meine bereits vorhandenen Kenntnisse und Fähigkeiten, welche ich ebenfalls in meiner Ausbildung zum
                Fachinformatiker erworben habe, vertiefen und ausbauen. Ich bin mir sicher, dass ich Ihr Team mit
                meinen Stärken erfolgreich unterstützen kann.</p>
            <p>Für meine Zukunft in diesem Beruf erhoffe ich mir in Ihrem Haus einen herausfordernden, kreativen und
                abwechslungsreichen Arbeitsplatz. Gerne würde ich Sie in einem persönlichen Gespräch davon überzeugen,
                dass eine Tätigkeit in Ihrem Hause genau meinen Vorstellungen entspricht
                und dies der Sinn meiner beruflichen Zukunft darstellt.</p>',

    '[APPLICATION_PERSONAL]' => 'Perönliches',
    '[APPLICATION_NAME]' => 'Name',
    '[APPLICATION_FIRSTNAME]' => 'Vorname',
    '[APPLICATION_ADDRESS]' => 'Adresse',
    '[APPLICATION_MOBILE]' => 'Telefon',
    '[APPLICATION_BIRTHDAY]' => 'Geburtsdatum',
    '[APPLICATION_BIRTHDAY_MARCH]' => 'März',
    '[APPLICATION_CIVIL]' => 'Zivilstand',
    '[APPLICATION_CIVIL_TEXT]' =>'ledig',

    '[APPLICATION_WORK]' => 'Berufliche Erfahrungen',
    '[APPLICATION_WORK_1]' => '<td>09/2012 – heute</td>
                    <td>Unister GmbH Stralsund<br>
                        Junior Softwareentwickler
                    </td>',
    '[APPLICATION_WORK_2]' => ' <td>01/2011 – 04/2012</td>
                    <td>Cinestar Stralsund<br>
                        Studentische Nebentätigkeit
                    </td>',
    '[APPLICATION_WORK_3]' => '<td>09/2009 – 08/2010</td>
                    <td>IT-College Putbus gGmbH<br>
                        Praktikum in der Administration
                    </td>',

    '[APPLICATION_EDUCATION]' => 'Ausbildung',
    '[APPLICATION_EDUCATION_1]' => '<td>09/2010 – 08/2012</td>
                    <td>Fachhochschule Stralsund<br>
                        Angewandte Informatik – <br>
                        Informations- und Kommunikationstechnik
                    </td>',
    '[APPLICATION_EDUCATION_2]' => '<td>09/2009 – 08/2010</td>
                    <td>IT-College Putbus gGmbH<br>
                        Ausbildung Fachinformatiker / <br>
                        Anwendungsentwicklung
                    </td>',
    '[APPLICATION_EDUCATION_3]' => ' <td>07/2007 – 08/2009</td>
                    <td>IT-College Putbus gGmbH <br>
                        Ausbildung staatlich geprüfter <br>
                        Technischer Assistent / <br>
                        Fachhochschulreife
                    </td>',


    '[APPLICATION_QUALIFICATION]' => 'Qualifikationen',
    '[APPLICATION_QUALIFICATION_LANG]' => '<td>Sprachen</td>
                    <td>Deutsch(Muttersprache)<br>
                        Englisch
                    </td>',
    '[APPLICATION_QUALIFICATION_DIFFERENT]' => '<td>Verschiedenes:</td>
                    <td>
                        gute Windowskenntnisse<br>
                        gute Linuxkenntnisse<br>
                        Umgang mit Microsoft Office, Libre/Open Office
                    </td>',
    '[APPLICATION_QUALIFICATION_HUMAN]' => '<td>Menschliches:</td>
                    <td>
                        Strukturierte Arbeitsweise<br>
                        Teamfähigkeit<br>
                        Kreative Ideen<br>
                        Organisationstalent<br>
                        Stressrobust
                    </td>',

);